  ...
  <AnimatePresence>
        {isModalOpen && (
          <motion.div
            ...
            transition={{ duration: 0.5 }}
            className="fixed top-1/2 right-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white w-[48%] rounded-lg shadow-lg p-4 flex flex-col items-center h-1/2 leading-5 text-[15px] font-normal" onScroll={handleCloseModal}
          >
             {selectedTender && (
    <>
      <h2 className="mb-4">{selectedTender.name}</h2>
      <p>Reference Number: {selectedTender.referenceNumber}</p>
      <p>Details of Publication in the Print Media: {selectedTender.printMedia}</p>
      ...
      <p>Date & Time of Opening of Bids: {selectedTender.openingBidsDate}</p>
      <p>Date & Time of Opening of Financial Bids: {selectedTender.openingFinancialBidsDate}</p>
